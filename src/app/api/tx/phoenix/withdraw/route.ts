...
import { loadPhoenixExchange, phoenixWithdrawIx } from "@/server/tx/phoenix";
...
export const POST = handlePost(phoenixAmountBody, async ({ payer, vault, amount }) => {
  ...
  return assemble(authority, [await phoenixWithdrawIx(getProgram(), ctx, authority, ex, new BN(amount))]);
});
