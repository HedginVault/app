    ...
    >
      ...
      <Field label={`Amount (${v.depositSymbol})`} error={error} hint={`Wallet balance ${formatTokenAmount(position.depositTokenBalance, v.depositDecimals)} ${v.depositSymbol}`}>
        <div className="relative">
          ...
          <button type="button" disabled={!!closed} className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md px-2 py-1 text-[12px] font-medium text-sky-400 hover:bg-accent-soft disabled:text-white/40 disabled:hover:bg-transparent" onClick={() => setInput(formatTokenAmount(position.depositTokenBalance, v.depositDecimals).replace(/,/g, ""))}>
            Max
          </button>
        </div>
      </Field>
